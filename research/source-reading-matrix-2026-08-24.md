...
首轮检索候选总量：知乎 111、小红书 102、抖音 63、公众号索引 55。平台候选已经达到“每个平台至少 50 条”。截至本次更新，完成正文精读的平台材料为知乎 50 篇、公众号 39 篇、小红书 2 篇、抖音 50 篇；本矩阵会持续将实际精读状态逐条落档，不能将候选数误报为已全文精读数。精确进度及去重口径见 `reading-progress-2026-08-24.md`。
...
第 5、6 页在本轮仍可读取索引标题和摘要，但点击索引链接后页面停留在搜狗结果页；直接请求 `weixin.sogou.com/link` 也只返回搜狗通用页，没有可核验的 `mp.weixin.qq.com` 正文。页面 6 的候选（如“22条自查表+5套提示词模板”“会自己学习文风的公众号 Skill”等）因此继续列为候选，不计入 39 篇正文精读。
